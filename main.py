...
# Consolidated Memory and Database layer for the Remake
try:
    ...
    class SQLiteAgentMemory:
        ...
        def _init_db(self):
            with self._get_connection() as conn:
                conn.executescript("""
                    CREATE VIRTUAL TABLE IF NOT EXISTS agent_memory USING fts5(task_prompt, blueprint, code, status, engine, created_at UNINDEXED);
                    CREATE TABLE IF NOT EXISTS chat_messages(id TEXT PRIMARY KEY, role TEXT, content TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
                """)
